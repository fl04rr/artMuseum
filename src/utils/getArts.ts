...
export const getArtsByPage = async (
  page: number,
): Promise<ArtResponse["data"] | undefined> => {
  return fetchData(
    `${process.env.REACT_APP_BASE_URL}?page=${page}&limit=${ARTS_LIMIT}`,
  );
};

export const getFavoritesArts = async (
  ids: number[],
): Promise<ArtResponse["data"] | undefined> => {
  return fetchData(
...
export const getArtById = async (
  artId: string,
): Promise<SingleResponse["data"] | undefined> => {
